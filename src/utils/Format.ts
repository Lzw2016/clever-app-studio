 ...
 */
const dateSubtract = (howLong: number, unit: ManipulateType, date?: any): dayjs.Dayjs => {
 ...
 * @param date      被运算的时间(默认当前时间)
 */
const dateAdd = (howLong: number, unit: ManipulateType, date?: any): dayjs.Dayjs => {
    if (!date) date = new Date();
...
const howLongAgo = (agoDate: Date, level?: number, maxTimeStamp?: number, timeUnit?: string[]): string => {
    ...
    let ct = new Date().getTime() - toDayjs(agoDate)!.toDate().getTime();
